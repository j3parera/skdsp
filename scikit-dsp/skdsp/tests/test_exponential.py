...
class ExponentialTest(unittest.TestCase):
    ...
    def test_yexpr_real_imag(self):
        ...
        '''
        # exponencial discreta
        c = ds.Exponential(ds.n, 2, sp.exp(sp.I*10))
        # expresión
    ...
    def test_dtype(self):
        ...
        for b in np.arange(-2, 0):
            c = ds.Exponential(ds.n, 1, b)
            self.assertFalse(c.dtype_is_complex)
            with self.assertRaises(ValueError):
                c.dtype = np.int_
        for b in np.arange(0, 3):
            ...
            with self.assertRaises(ValueError):
                c.dtype = np.int_
        c = ds.Exponential(ds.n, 1, 1+1j)
        self.assertTrue(c.dtype_is_complex)
        c = ds.Exponential(ds.n, 1, 3*sp.exp(sp.I*sp.S.Pi/4))
        self.assertTrue(c.dtype_is_complex)
    ...
    def test_period(self):
        ...
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*sp.S.Pi/4))
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 8)
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*3*sp.S.Pi/8))
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 48)
        c = ds.Exponential(ds.n, 1, sp.exp(-sp.I*3*sp.S.Pi/8))
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 208)
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*0.83*sp.S.Pi))
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 16600)
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*3/8))
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*1/4))
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        c = ds.Exponential(ds.n, 1, sp.exp(sp.I*3/2))
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        # unos cuantos casos, p.ej 0.35*pi -> 40, 0.75*pi -> 8
        for M in np.arange(5, 105, 5):
            f = Fraction(200, M)
            c = ds.Exponential(ds.n, 1, sp.exp(sp.I*M/100*sp.S.Pi))
    ...
    def test_repr_str_latex(self):
        ...
        '''
        # exponencial discreta
        c = ds.Exponential(ds.n, 3, sp.exp(sp.I*sp.S.Pi/4))
        # repr
        self.assertEqual(repr(c), 'Exponential(n, 3, exp(j*pi/4))')
        # str
        ...
        self.assertEqual(latex(c, mode='inline'),
                         r'$3 {\mathrm{e}}^{-{\mathrm{j}}(\pi / 4)n}$')
        # exponencial discreta
        c = ds.Exponential(ds.n, -5, sp.exp(sp.I*10))
        # repr
        self.assertEqual(repr(c), 'Exponential(n, -5, exp(10*j))')
        # str
        self.assertEqual(str(c), '-5*exp(10*j*n)')
        # latex
        self.assertEqual(latex(c, mode='inline'),
                         r'$-5 {\mathrm{e}}^{\,{\mathrm{j}}(10)n}$')
        c = ds.Exponential(ds.n, 1, -1)
        # repr
        self.assertEqual(repr(c), 'Exponential(n, 1, -1)')
        # str
        self.assertEqual(str(c), '(-1)**n')
        # latex
        self.assertEqual(latex(c, mode='inline'), r'$\left(-1\right)^n$')
        c = ds.Exponential(ds.n, 3, sp.exp(sp.I*sp.S.Pi/4)).flip().delay(2)
        # repr
        self.assertEqual(repr(c), 'Exponential(-n + 2, 3, exp(j*pi/4))')
        # str
        self.assertEqual(str(c), '3*exp(-j*pi*(n - 2)/4)')
        # latex
        self.assertEqual(latex(c, mode='inline'),
                         r'$3 {\mathrm{e}}^{\,{\mathrm{j}}(\pi / 4)' +
                         r'(- n + 2)}$')
        c = ds.Exponential(ds.n, 1+1j, sp.exp(sp.I*sp.S.Pi/4)).flip().delay(2)
        # repr
        self.assertEqual(repr(c), 'Exponential(-n + 2, (1+1j), exp(j*pi/4))')
        # str
        self.assertEqual(str(c), '(1.0 + 1.0*j)*exp(-j*pi*(n - 2)/4)')
        # latex
        self.assertEqual(latex(c, mode='inline'),
                         (r'$(1+1j) {\mathrm{e}}^{\,{\mathrm{j}}(\pi / 4)' +
                          r'(- n + 2)}$'))
    ...
    def test_generator(self):
        ...
        c = ds.Exponential(ds.n, 2, sp.Rational(1, 2))
        with self.assertRaises(ValueError):
    ...
    def test_shift_delay(self):
        ...
        with self.assertRaises(ValueError):
            d.shift(0.5)
        d = ds.Exponential(ds.n, 1, 2).shift(2)
        np.testing.assert_array_equal(d[-3:3], np.power(2.0, np.arange(-5, 1)))
        with self.assertRaises(ValueError):
            d.delay(0.5)
        d = ds.Exponential(ds.n, 1, sp.Rational(1, 4)).delay(-2)
        np.testing.assert_array_equal(d[-3:3],
    ...
    def test_scale(self):
        ...
        with self.assertRaises(ValueError):
            d.scale(sp.pi)
        d = ds.Exponential(ds.n, 1, sp.Rational(1, 2)).scale(3)
        np.testing.assert_array_equal(d[-3:3],
                                      np.power(0.5, np.arange(-3, 3)))
        d = ds.Exponential(ds.n, 1, sp.Rational(1, 2)).scale(0.75)
        np.testing.assert_array_equal(d[-12:12:4],
                                      np.power(0.5, np.arange(-12, 12, 4)))
        d = ds.Exponential(ds.n, 1, sp.Rational(1, 2)).scale(1.5)
        np.testing.assert_array_equal(d[-12:12:4],
                                      np.power(0.5, np.arange(-12, 12, 4)))
    ...
    def test_amplitude(self):
        ...
        # exponencial discreta
        s = ds.Exponential(ds.n, 3, 2)
        self.assertEqual(3, s.amplitude)
        s = ds.Exponential(ds.n, 4, 2)
        self.assertEqual(4, s.amplitude)
        s = ds.Exponential(ds.n, (3+3j), 2)
    ...
    def test_real_exponential(self):
        ...
        self.assertFalse(c.is_continuous)
        n = ds.n
        c = ds.RealExponential(n, 1, -1)
        self.assertIsNotNone(c)
        c = ds.RealExponential(n, 2, 0.5)
        self.assertIsNotNone(c)
        with self.assertRaises(ValueError):
            ds.RealExponential(n, 3, 2+2j)
        with self.assertRaises(ValueError):
            ds.RealExponential(n, 3+3j, 2)
        with self.assertRaises(ValueError):
            ds.RealExponential(n, 3+3j, 2+2j)
