...
class ExponentialTest(unittest.TestCase):
    ...
    def test_period(self):
        ...
        c = ds.Exponential(1, sp.S.Pi/4)
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 8)
        c = ds.Exponential(1, 3*sp.S.Pi/8)
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 16)
        c = ds.Exponential(1, -3*sp.S.Pi/8)
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 16)
        c = ds.Exponential(1, 0.83*sp.S.Pi)
        self.assertTrue(c.is_periodic)
        self.assertEqual(c.period, 200)
        c = ds.Exponential(1, 3/8)
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        c = ds.Exponential(1, 1/4)
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        c = ds.Exponential(1, 3/2)
        self.assertFalse(c.is_periodic)
        self.assertEqual(c.period, sp.oo)
        # unos cuantos casos, p.ej 0.35*pi -> 40, 0.75*pi -> 8
        for M in np.arange(5, 105, 5):
            f = Fraction(200, M)
            c = ds.Exponential(1, M/100*sp.S.Pi)
    ...
    def test_repr_str_latex(self):
        ...
        '''
        # exponencial discreta
        c = ds.Exponential(3, sp.S.Pi/4, sp.S.Pi/8)
        # repr
        self.assertEqual(repr(c), 'Exponential(3, pi/4, pi/8)')
        # str
        self.assertEqual(str(c), '3*cos(pi/4*n + pi/8)')
        # latex
        self.assertEqual(c.latex_yexpr(), (r'3 \cos{\left (\frac{\pi n}{4}' +
                                           r' + \frac{\pi}{8} \right )}'))
        # exponencial discreta
        c = ds.Exponential(-5, 10)
        # repr
        self.assertEqual(repr(c), 'Exponential(-5, 10, 0)')
        # str
        self.assertEqual(str(c), '-5*cos(10*n)')
        # latex
        self.assertEqual(c.latex_yexpr(),
                         r'- 5 \cos{\left (10 n \right )}')
        c = ds.Exponential(3, sp.S.Pi/4, sp.S.Pi/8).flip().delay(2)
        # repr
        self.assertEqual(repr(c), 'Exponential(3, pi/4, pi/8)')
        # str
        self.assertEqual(str(c), '3*cos(pi/4*(-n + 2) + pi/8)')
        # latex
        self.assertEqual(c.latex_yexpr(), (r'3 \sin{\left (\frac{\pi}{4}' +
                                           r' \left(n - 2\right)' +
                                           r' + \frac{3 \pi}{8} \right )}'))
