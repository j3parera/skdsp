...
from ..operator.operator import (AbsOperator, ConjugateOperator, FlipOperator,
                                 HermitianOperator, ImaginaryPartOperator,
                                 RealPartOperator, ScaleOperator,
                                 ShiftOperator)
...
class _Signal(sp.Basic):
    ...
    def __new__(cls, *args, domain=sp.S.Reals, codomain=sp.S.Reals):
        """
        Signal creation.
        """
        ...
        return sp.Basic.__new__(cls, *args, domain, codomain)
    ...
    def latex_name(self):
        ...
        if m:
            s = r'{0}_{{{1}}}'.format(m.group(1), m.group(2))
        else:
            s = self._name
        return s
    ...
    def __eq__(self, other):
        ...
        if not equal:
            return False
        return (
            sp.Eq(other._xexpr.xreplace({other._xvar: self._xvar}) - self._xexpr, 0)
            == sp.S.true
        )
...
class _FunctionSignal(_Signal):
    ...
    def __eq__(self, other):
        if not super().__eq__(other):
            return False
        return (
            sp.Eq(other._yexpr.xreplace({other._xvar: self._xvar}) - self._yexpr, 0)
            == sp.S.true
        )

    @property
    def is_even(self):
        return sp.Eq(self._yexpr, self._yexpr.xreplace({self._xvar: -self._xvar}))
