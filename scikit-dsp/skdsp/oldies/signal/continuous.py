...
class _ContinuousMixin(object):
    ...
    def period(self, value):
        v = sp.sympify(value)
        if _is_real_scalar(v) or v.equals(sp.oo) or isinstance(v, sp.Symbol):
            self._period = v
        else:
    ...
    def _sympify_xexpr(cls, expr):
        ...
        if not var.is_real or var.is_integer or not expr.is_real or expr.is_integer:
    ...
    def _check_indexes(self, x):
        ...
        if not np.all(np.isreal(x)):
            raise ValueError("continuous signals are only defined" + "for real indexes")
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, _ContinuousMixin):
            raise TypeError("can't add continuous signal and {0}".format(type(other)))
    ...
    def __mul__(self, other):
        if isinstance(other, Number):
            other = Constant(other)
        if not isinstance(other, _ContinuousMixin):
            raise TypeError("can't mul continuous signal and {0}".format(type(other)))
    ...
    def __truediv__(self, other):
        if isinstance(other, Number):
            other = Constant(other)
        if not isinstance(other, _ContinuousMixin):
            raise TypeError("can't div continuous signal and {0}".format(type(other)))
...
class Constant(ContinuousFunctionSignal):
    ...
    def __init__(self, const=0, **kwargs):
        ...
        ContinuousFunctionSignal.__init__(
            self, _ContinuousMixin.default_xvar(), const, **kwargs
        )
    ...
    def __str__(self, *_args, **_kwargs):
        re, im = self._yexpr.as_real_imag()
        if im == 0:
            s = str(re)
        elif re == 0:
            s = str(self._yexpr).replace(r"*j", "j")
        else:
            s = str(self._yexpr).replace(" ", "").replace(r"*j", "j")
        return s

    def __repr__(self):
        return "Constant({0})".format(str(self))

    def _siglatex(self, printer, **_kwargs):
        return str(self).replace("j", r"\mathrm{j}")
