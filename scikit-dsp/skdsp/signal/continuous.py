...
from skdsp.util.util import as_coeff_polar, ipyplot
...
class ContinuousSignal(Signal):
    ...
    def __new__(cls, amp, iv, codomain, **kwargs):
        if iv is not None and (not iv.is_symbol or not iv.is_real):
            raise ValueError("Invalid independent variable.")
        return Signal.__new__(
            cls, amp, iv, sp.S.Reals, codomain, **kwargs
        )
    ...
    def ipyplot(
        ...
        **kwargs
        ...
        return ipyplot(n, v, xlabel, title, axis, color)
...
class Undefined(ContinuousSignal):
    ...
    def _match_expression(cls, expr, **kwargs):
        ...
        if not expr.has(AppliedUndef):
            return None
        iv = kwargs.pop("iv", ContinuousSignal._default_iv)
    ...
    def __new__(
        ...
        iv = sp.sympify(iv) if iv is not None else ContinuousSignal._default_iv
        if not isinstance(name, str):
            raise ValueError("Name must be a string.")
        undef = sp.Function(name, nargs=1)(iv)
        return Signal.__new__(cls, undef, iv, sp.S.Reals, codomain, **kwargs)
